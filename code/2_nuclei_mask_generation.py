...
def find_nuclei(nuclei_folders: list) -> list:
    ...
    for nuclei_folder in nuclei_folders:
        ...
        # Setting up logging
        file_handler = logging.FileHandler(os.path.join(output_folder, '2_log.txt'), mode='w')
        ...
        if not image_files:
            logging.error(f"No .tif images found in folder '{nuclei_folder}'. Skipping folder.")
            continue
        ...
        for image_file in image_files:
            ...
            if image.dtype != np.uint8:
                logging.error(f"Image '{image_file}' is not 8-bit grayscale. Skipping file.")
                continue
            ...
            labels, details = model.predict_instances(image, nms_thresh=0.9, prob_thresh=0.7)
...
def process_nuclei(valid_folders: list, particle_size: int) -> None:
    """
    ...
    for input_folder in valid_folders:
        ...
        # Create a new folder 'Final_Nuclei_Mask_<timestamp>' in the same directory as input_folder
        processed_folder = os.path.join(os.path.dirname(input_folder), f'Final_Nuclei_Mask_{current_datetime}')
        ...
        for filename in os.listdir(input_folder):
            ...
            if imp is None:
                logging.warning(f"Failed to open image: {file_path}. Check Bio-Formats or file integrity.")
                continue
...
def main(input_json_path: str, particle_size: int) -> None:
    """
