...
def build_intersection_mask(*masks):
    """
    Creates a labeled mask representing the intersection of all input labeled masks.
    Each distinct overlap region among them gets a unique label.
    """
    if len(masks) < 2:
        raise ValueError("build_intersection_mask requires at least 2 masks.")

    shape_set = {m.shape for m in masks}
    if len(shape_set) != 1:
        raise ValueError("All masks must have the same shape for intersection.")

    intersection_mask = np.zeros_like(masks[0], dtype=np.uint16)
    label_counter = 1
    ...
    # use itertools.product to combine possible labels from each mask
...
def count_foci_in_nuclei(nuclei_mask, foci_mask, pixel_area, image_key):
    """
    Counts how many labeled objects in 'foci_mask' fall inside each labeled nucleus in 'nuclei_mask'.
    Returns a list of dicts with metrics: Foci Count, total area, relative area, etc.
    """
...
def process_nuclei_image(nuc_file_path: str,
    ...
    Args:
      nuc_file_path, foci_channels_info, metadata, results_folder, perform_colocalization
      min_subset_size, max_subset_size define which channel subsets we intersect
    Returns:
      A list of dictionaries with final results for each nucleus.
    """
    nuc_filename = os.path.basename(nuc_file_path)
    nuc_key = extract_image_key(nuc_filename)
    logging.info(f"Started nuclei processing: {nuc_filename}")
...
def parallel_processing(nuclei_files: list,
    ...
    logging.info(f"Total tasks (nuclei files): {total_tasks}. Starting pool with max_workers={max_workers}.")
...
if __name__ == "__main__":
    ...
    # Switch the start method to "spawn" to avoid potential issues with 'fork'
    multiprocessing.set_start_method("spawn", force=True)

    parser = argparse.ArgumentParser(
        description="Parallel foci/nuclei analysis with optional universal colocalization and black-objects/white-bg masks."
    )
